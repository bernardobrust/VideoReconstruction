...
typedef struct {
  ...
  alignas(16) char read_buf[8192];
  unsigned long read_buf_len;
} InternalState;

// Internal functions
// ----------------------------------------------------------------
static int display_connect() {
...
static unsigned wl_display_get_registry(int fd) {
...
static unsigned wl_registry_bind(int fd, unsigned registry, unsigned name,
                                 char *interface, unsigned interface_len,
                                 unsigned version) {
  unsigned long msg_size = 0;
  char msg[512] = "";
...
static unsigned wl_compositor_create_surface(int fd, InternalState *state) {
...
static void create_shared_memory_file(unsigned long size,
                                      InternalState *state) {
  char name[255] = "/";
...
static void xdg_wm_base_pong(int fd, InternalState *state, unsigned ping) {
...
static void xdg_surface_ack_configure(int fd, InternalState *state,
                                      unsigned configure) {
  ...
  buf_write_u16(msg, &msg_size, sizeof(msg), xdg_surface_ack_configure_opcode);
...
static unsigned wl_shm_create_pool(int fd, InternalState *state) {
...
static unsigned xdg_wm_base_get_xdg_surface(int fd, InternalState *state) {
  assert(state->xdg_wm_base > 0);
...
static unsigned wl_shm_pool_create_buffer(int fd, InternalState *state) {
  ...
  buf_write_u16(msg, &msg_size, sizeof(msg), wl_shm_pool_create_buffer_opcode);
...
static void wl_surface_attach(int fd, InternalState *state) {
  assert(state->wl_surface > 0);
...
static void wl_surface_damage(int fd, InternalState *state) {
  assert(state->wl_surface > 0);
...
static unsigned xdg_surface_get_toplevel(int fd, InternalState *state) {
  assert(state->xdg_surface > 0);
...
static void wl_surface_commit(int fd, InternalState *state) {
  assert(state->wl_surface > 0);
...
static void xdg_toplevel_set_title(int fd, InternalState *state,
                                   const char *title) {
...
static unsigned wl_seat_get_keyboard(int fd, InternalState *state) {
...
static void handle_message(int fd, PlatformState *platform_state, char **msg,
  ...
  if (object_id == state->wl_registry && opcode == wl_registry_event_global) {
    ...
    if (strcmp(wl_shm_interface, interface) == 0)
      state->wl_shm = wl_registry_bind(fd, state->wl_registry, name, interface,
                                       interface_len, version);

    char xdg_wm_base_interface[] = "xdg_wm_base";

    if (strcmp(xdg_wm_base_interface, interface) == 0)
      state->xdg_wm_base = wl_registry_bind(fd, state->wl_registry, name,
    ...
    if (strcmp(wl_seat_interface, interface) == 0)
      state->wl_seat = wl_registry_bind(fd, state->wl_registry, name, interface,
                                        interface_len, version);

    return;
  } else if (object_id == display_object_id &&
             opcode == wl_display_error_event) {
    ...
    [[maybe_unused]] unsigned format = buf_read_u32(msg, msg_len);

    return;
  } else if (object_id == state->wl_buffer &&
             opcode == wl_buffer_event_release) {
    return;
  } else if (object_id == state->xdg_wm_base &&
             opcode == xdg_wm_base_event_ping) {
    ...
    if (opcode == xdg_toplevel_event_configure) {
      [[maybe_unused]] unsigned w = buf_read_u32(msg, msg_len);
      [[maybe_unused]] unsigned h = buf_read_u32(msg, msg_len);
    ...
    if (opcode == 1) {
      ...
      *msg_len -= ROUNDUP_4(keys_len);
      ...
      buf_read_u32(msg, msg_len);
      buf_read_u32(msg, msg_len);
      [[maybe_unused]] unsigned key = buf_read_u32(msg, msg_len);
      [[maybe_unused]] unsigned key_state = buf_read_u32(msg, msg_len);
...
static void read_and_dispatch(PlatformState *platform_state, bool block) {
  InternalState *state = (InternalState *)platform_state->internal_state;

  while (true) {
    long bytes_received = recv(state->fd, state->read_buf + state->read_buf_len,
